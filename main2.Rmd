...
# plot(rumi, vertex.label=paste0(V(rumi)$subscale, " \n", V(rumi)$label), edge.label=round(E(rumi)$strength,3))
```
...
#### 2.1.2 Removing redundant items
...
The RRS (order: `r vcount(RRS_graph)`, size: `r ecount(RRS_graph)`) and SCRS (order: `r vcount(SCRS_graph)`, size: `r ecount(SCRS_graph)`) networks.

```{r }
#TODO: proper seed, clusternig
seed <- 73632
summary_table <- bind_rows(network_summary(RRS_graph, name="RRS",seed=seed),
                            network_summary(SCRS_graph, name="SCRS", seed=seed),
                            network_summary(rumi, name="Combined RRS+SCRS",seed=seed))
