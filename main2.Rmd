...
Having two rumination-related tools, we assumed that network analysis will reveal

* firstly, their respective internal structure previously established in the literature (RRS having a two-scale structure: reflection and brooding; while SCRS will show a single-scale structure);
* secondly, we hypothesized that in their common network items overlapping in scope will change these clear outlines of the two measurement tools.
...
In the current study, we used a fairly large sample of indiviuals (N=`r nrow(rumi_data)`) to construct a dense, undirected network of psychological constructs. 
...
#### 2.1.1 Network Analysis with LASSO

We constructed indiviudal and combined networks of RRS and SCRS with respective penalized correlation matrices using the *EBICglasso* function of the **qgraph** package (number of lambda parameters were set to 1000).
...
The LASSO uses L1 regularization, and in the current study, it results in a partial correlation network where edges represent direct associations between pairs of items, while the influence of all other items in the network are kept controlled. 
...
# RRS_goldbricker <- goldbricker(RRS_subset, p = 0.05, method = "hittner2003", threshold = 0.25, corMin = 0.75, progressbar = TRUE)
...
# SCRS_goldbricker <- goldbricker(SCRS_subset, p = 0.05, method = "hittner2003", threshold = 0.25, corMin = 0.75, progressbar = TRUE)
# saveRDS(SCRS_goldbricker, "output/SCRS_goldbricker.RDS")
# SCRS_goldbricker <- readRDS("output/SCRS_goldbricker.RDS")

# combined
# combined_goldbricker <- goldbricker(dataset, p = 0.05, method = "hittner2003", threshold = 0.25, corMin = 0.75, progressbar = TRUE)
# saveRDS(combined_goldbricker, "output/combined_goldbricker.RDS")
# combined_goldbricker <- readRDS("output/combined_goldbricker.RDS")
