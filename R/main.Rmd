...
In order to get an understanding of community of nodes and to see if nodes can be grouped into neighbourhoods, we used the *igraph* R package. When detecting possible communities of nodes, the detection happened with the following parameters in the *sping lass algorithm* [@Reichardt2006; @Bernstein2019].
...
the centrality indices and edge-weights.
 
## 3. Results

### 3.1 Network plots
When estimating networks, we modeled RRS, SCRS and their combined network as Gaussian graph-networks which are networks of partial correlation coefficients [@Epskamp2018]. In many parts of this analysis we also utilized functions from the *qgaph* R package [@Epskamp2012].
...
RRS items are grouped into two communities. In one we find the strongly connected items *alone1* and *alone2* with items *write*, *person* and *analyze*. The rest of the items constitute the other main community.
...
The self-critical rumination items exhibit one large community as a whole.

RRS and SCRS combined communities identified with parameters *gamma = 0.5, spins = 25, starting temperature = 1, ending temperature = 0.01, cooling factor = 0.99*.
...
When all RRS and SCRS items are handled as one covariance and one Gaussian graph-network, 4 items of RRS (*handle*, *problem*, *deserve* and *wish*) are grouped into the community of the SCRS items.
...
Items such as *alone1*, *alone2* and *person* are high in terms of expected influence, meaning that these items play more central role in the RRS network, by changing these nodes, the rest of the network changes more likely.
...
SCRS-tems such as *habits* and *thoughts* are more influencing nodes than *productive*.
...
In the combined network scenario, the RRS item *alone1*, the SCRS items *habits* and *thoughts* maintain their influencing position among the items. Due to the combination of tools, the SCRS item *different* has increased its influence, and the items *mistakes and aspects* have increased their impact as a PCA component. This also means that in the combined network they proved to be too redundant and the *goldbricker* algorithm used their PCA-component instead.
...
We used the *bootnet* R package to have an estimate of edge-weight accuracy with *nonparametric bootstrap* (2500 times sampling) and also produce measures of centrality stability with *case dropping bootstrap* (to simulate cases where only the subset of the existing data would be available).
...
Stability estimates of the RRS network centrality indices
...
Network accuracy estimate for combined RRS and SCRS

```{r echo=FALSE, fig.height=20, fig.width=14, message=FALSE, warning=FALSE}
# generate the accuracy estimate and save it
