...
#CS
corStability(RRS_stability)

network <- RRS_stability
network_stability_plot <- function(network) {
    B_levels = seq(1, 2500)
    sample_data  <- network$sampleTable
    boot_data <- network$bootTable  %>%
    ...
    for (b in B_levels) {
        for (level in drop_levels) {
            for (m in measures) {
                i = i + 1
                boot_sample  <- boot_data %>%
                    filter(
                           bootstrap_indicator == b,
                           type == m,
                           nPerson == level
                    )
                ...
                original_sample  <- sample_data %>%
                    filter(type == m)
                test  <-all(!is.null(nrow(boot_sample)), !is.null(nrow(original_sample)), length(original_sample$value) == length(boot_sample$value)) 
                if (test) {
                    correlation = cor(boot_sample$value, original_sample$value)
                    new_record <- tibble(
                                         b = b,
                                         measure = m,
                                         level = level,
                    ...
                    plot_data <- bind_rows(plot_data, new_record)
    ...
    plot_data %>%
        ggplot(aes(reorder(sample_rate, desc(sample_rate)), value, group = type)) +
