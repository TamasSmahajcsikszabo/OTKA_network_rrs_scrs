...
In order to map the relationships of the actual RRS and SCRS items, both on their respective networks, and in a combined network, we used the LASSO algorithm [@Friedman2007]. The LASSO uses L1 regularization, and results in a partial correlation network where edges represent direct associations between pairs of items, while the influence of all other items in the network kept controlled. The regularization by L1 penalty ensures that trivially small partial correlations are set to zero [@McNally_2017]. The tuning parameter of *gamma* in the graphical LASSO algorithm has been set to 0.5 value in accordance with [@McNally_2017] and [@Bernstein2019] in order to achieve sparse graphs. The final networks were chosen by the lowest values of the *extended Bayesian Information Criterion* (EBIC).
