...
expected_inf_plot <- function(network, toolname) {
  ...
  influence_tibble <- tibble(
    OneStep = step1,
    TwoStep = step2,
    item = as_factor(paste0(toolname, "_", seq(1, length(step1))))
...
make_bridge_plot <- function(network, toolname) {
  ...
  # create plot
  reshaped_output %>%
    ggplot(aes(item, value, color = Community, shape = Community)) +
...
In order to further elaborate the question of importance within the network, we identified *bridge nodes* using the functions provided by the *networktools* R package (Jones, 2018). We estimated both one-step and two-step bridge expected influence measures which reflect the sum of edge weights connecting one node to other groups on nodes (or communities of nodes); and also express a node's secondary influence on the rest of the network, respectively. Higher influence scores indicate that a certain node is more likely to active neighbouring nodes, subgroups of nodes or communities.
...
## References
Jones, P. (2018). Networktools: Tools for Identifying Important Nodes in Networks. R Package Version 1.2.3.
